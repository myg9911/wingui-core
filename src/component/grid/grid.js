...
import { progressSpinner } from '@zionex/wingui-viewconfig/src';
...
export function applyGridContextMenu(gridView, { username, systemAdmin }, opneLayoutDlg, layoutDt) {
  if (layoutDt) {
...
function getFlatColumns(gridView, allColumns, layout, flatCols, hiddenCols) {
  for (let i = 0; i < layout.length; i++) {
    const el = layout[i]
    if (el.items) {
      getFlatColumns(gridView, allColumns, el.items, flatCols, hiddenCols)
    }
    else {
      const column = gridView.columnByName(el.column)
      if (allColumns || column.visible) {
        flatCols.push(el.column)
      }
      if (allColumns && !column.visible) {
        hiddenCols.push(el.column)
      }
    }
  }
}

export function getFieldsByDisplayOrder(gridView, allColumns) {
  const flatCols = []
  const hiddenCols = []
  let layout = gridView.saveColumnLayout();
  getFlatColumns(gridView, allColumns, layout, flatCols, hiddenCols);
  return [flatCols, hiddenCols];
}
...
export function setColorPickerRenderer(gridView, columnName, options) {
    ...
    gridView.registerCustomRenderer(colorPickerRenderer, {
      ...
      render: function (grid, model, width, height, info) {
        ...
        $(input).minicolors && $(input).minicolors({
          control: settings.control || 'hue',
          format: 'hex',
          inline: false,
          letterCase: settings.letterCase || 'lowercase',
          opacity: settings.opacity || false,
...
TODimArray.prototype._getRow = function (row) {
  if (!this.relatedRowIdx) {
    this.relatedRowIdx = []
  }
  let rowIdx;
  if (typeof row == 'number') {
    ...
    let idx = this.relatedRowIdx.findIndex(v => v === row)
    if (idx >= 0) {
      rowIdx = idx;
    }
    else {
      idx = this.relatedRowIdx.push(row)
      rowIdx = idx;
    }
  }
  return rowIdx;
...
TODimArray.prototype._getCol = function (col) {
  let colIdx;
  if (!this.relatedColIdx) {
    this.relatedColIdx = []
  }
  if (typeof col == 'number') {
    ...
    let idx = this.relatedColIdx.findIndex(v => v === col)
    if (idx >= 0) {
      colIdx = idx;
    }
    else {
      idx = this.relatedColIdx.push(col) - 1
      colIdx = idx;
    }
  }
  return colIdx
...
TODimArray.prototype._add = function (row, col, obj) {
  if (this.dataArr[row] == undefined)
    this.dataArr[row] = []
  this.dataArr[row][col] = obj
}

TODimArray.prototype._remove = function (row, col) {
  if (this.dataArr[row] == undefined)
    return;

  const colArr = this.dataArr[row]
  colArr.splice(col, 1);
}

TODimArray.prototype._get = function (row, col) {
  if (this.dataArr[row] == undefined)
    return undefined;
...
TODimArray.prototype.add = function (row, col, obj) {
  if (!this.dataArr)
    this.dataArr = []
  let rowIdx = this._getRow(row);
  let colIdx = this._getCol(col);

  this._add(rowIdx, colIdx, obj)
}

TODimArray.prototype.remove = function (row, col) {
  if (!this.dataArr)
    return;
  ...
  this._remove(rowIdx, colIdx)
}

TODimArray.prototype.get = function (row, col) {
  if (!this.dataArr)
    return undefined;
  ...
  return this._get(rowIdx, colIdx)
}

TODimArray.prototype.clearAll = function () {
  if (!this.dataArr)
    return;

  for (let i = 0; i < this.dataArr.length; i++) {
    let colArr = this.dataArr[i]
    if (colArr) {
...
export { TODimArray, progressSpinner };
