...
import { createUniqueKey, createCSSSelector, JSonToStyleString, deleteCSSSelector, isDateDayEqual, showMessage } from '../utils/common'
...
const CustomTexInput = React.forwardRef(({ value, onClick, readOnly, disabled, displaySize, label, variant, ...others }, ref) => {
  ...
  return (
    <TextField {...others}
      size="small"
      variant={variant}
      className={others.rangeyn == 'Y' ? `${classes.root} ${classes.inputZDateRange}` : `${classes.root} ${classes.inputZDate}`}
      ref={ref}
      value={value}
      hiddenLabel={true}
      label={label}
...
export const ZDateTimePicker = React.forwardRef((props, ref) => {
  ...
  const { value, onChange, useCustomTime, min, max, dateFormat, dateformat, placeholderText,
    readonly, readOnly, rangeyn, openTo, selectedDays, selectDayStyle, includeDaysOfWeek, label, variant, type,
    ...other } = props
  const [selectedDate, setSelectedDate] = useState(getDateValue(value))
  ...
  if (dateformat.length > 10) {
    transParam.showTimeSelect = true
  }
  ...
  if(datetimeSettings.showWeek) {
    transParam.showWeekNumbers = true
  }

  if (datetimeSettings.weekStartsOn !== undefined) {
    transParam.calendarStartdDay = datetimeSettings.weekStartsOn
  }
