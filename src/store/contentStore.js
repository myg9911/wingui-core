...
let t3smartScmStore = (set, get, api) => ({
  ...
  searchAreaExpanded: true,
  setSearchAreaExpanded: newVal => set({ searchAreaExpanded: newVal }),
  contentData: [], //tab views
  setContentData: (newVal, resetRemoveViewEvntYn) => _setContentData(get(), set, newVal, resetRemoveViewEvntYn),
...
async function _initMenu(state, set, mode) {
  ...
  menu.initView(menu.data)
  set({ menus: menu.data })
  state.refreshBookmarks();
}
function _setContentData(state, set, newVal, resetRemoveViewEvntYn) {
  if (resetRemoveViewEvntYn) {
    set({ contentData: newVal, rmViewDataEvnt: defRmViewData })
  } else {
    set({ contentData: newVal })
  }
}
...
function fnSetMenuType(set, val) {
  ...
  document.body.dataset.menuType = val
  set({ menuType: val })
}
