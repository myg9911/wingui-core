...
export function showMessage(title, contents, opt, func) {
  ...
  let options = {
    close: (opt && opt.close !== undefined) ? opt.close : true,
    closeX: (opt && opt.closeX !== undefined) ? opt.closeX : true,
  }
...
export function getAppSettings(name, defVal) {
  if (uiSettings[name] != undefined)
    return uiSettings[name]
  else
    return defVal;
}
...
export function loadComboList(params) {
  ...
  return axios({
    ...
    if (params.ALLFLAG) {
      array.push({ value: "ALL", label: transLangKey("ALL") });
    }
