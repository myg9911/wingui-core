...
import { isDeepOrderlessEqual } from '../../utils/common'
import wingui3 from './wingui3-custom'
import { zAxios } from '../../common/imports';
import {
...
import { themeSkin } from '@zionex/wingui-viewconfig/src';
...
function BaseGrid(props) {
  ...
  function setData(jsonData) {
    ...
    if (gridView.prefInfo || gridView.crossTabInfo) {
      ...
      if (props.grid_doAfterSetData) {
        props.grid_doAfterSetData(gridView, dataProvider, jsonData)
      }
    }
  }
  ...
  useEffect(() => {
    ...
    __realGrid.current.gridView.clearAllCustomStyles = clearAllCustomStyles
    ...
    applyGridContextMenu(gridView, { username, systemAdmin }, openLayoutDlg);
    return unmount;
  }, [])
  ...
  function setFieldsProps(props) {
    ...
    for (let i = 0, len = props.length; i < len; i++) {
      ...
      if (prop.dataType.toUpperCase() === 'BOOLEAN') {
        ...
        objField.datetimeFormat = prop.datetimeFormat ?? 'iso';
  ...
  function setColumnProp(prop) {
    ...
    if(objColumn.groupFooter !== undefined) {
      objColumn.groupFooter.text = transLangKey(objColumn.groupFooter.text)
    }
    if(objColumn.footer !== undefined) {
      objColumn.footer.text = transLangKey(objColumn.footer.text)
    }
  ...
  function setOptions(gridView, dataProvider) {
    let editorOptions = {};
    editorOptions.viewGridInside = false;
  ...
  function setCellStyles(gridView, rowIdx, colName, styleName) {
    if (!gridView.customStyles) {
      gridView.customStyles = new TODimArray()
    }
  ...
  function clearAllCustomStyles(gridView) {
    if (gridView.customStyles) {
      gridView.customStyles.clearAll();
    }
  }
  ...
  function setCellStyle(gridView, rowIdx, colName, styleName) {
    ...
    let styleObj = gridView.customStyles.get(rowIdx, colName)
    if (styleObj == undefined) {
      ...
      gridView.customStyles.add(rowIdx, colName, style)
    } else {
      let style = styleObj
  ...
  function loadLayoutInfos(gridView, layouttype, defaultApply) {
    if (props.gridCd) {
      ...
      zAxios.get(baseURI() + 'system/users/grid-layouts', {
        ...
        applyGridContextMenu(gridView, { username, systemAdmin }, openLayoutDlg, layoutDt);
  ...
  const makeGridColumnWithPrefInfo = (gridView, dataProvider, resultData) => {
    if (props.onBeforeDataSet) {
      props.onBeforeDataSet(gridView, dataProvider, resultData)
    }
        ...
        if (groupHeaders.length > 0) {
          for (let groupHeadersIndex = 0, groupHeadersLen = groupHeaders.length; groupHeadersIndex < groupHeadersLen; groupHeadersIndex++) {
            ...
            for (let columnIdx = 0, arrangedColumnsLen = arrangedColumns.length; columnIdx < arrangedColumnsLen; columnIdx++) {
              ...
              if (getColumnProp(gridView, arrangedColumns[columnIdx], 'groupShowMode')) {
                groupShowMode = true;
              }
            }
            ...
            groupColumn.width = 0;
            if(groupShowMode) {
              groupColumn.expandable = true
              groupColumn.expanded = false
            }
    ...
    let targetResultData = resultData
    if (props.onReadyDataFill) {
      targetResultData = props.onReadyDataFill(gridView, dataProvider, resultData)
    }
    ...
    if (props.onAfterDataSet) {
      props.onAfterDataSet(gridView, dataProvider, resultData)
    }
  }
