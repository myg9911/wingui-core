...
import { waitOn, waitOff } from '@zionex/wingui-core/src/utils/waitMe'
...
zAxios.interceptors.request.use(
  function (config) {
    ...
    if(config.data) {
      if(config.data.has('timeout')){
        config.data.delete('timeout')
      }
      config.data.append('timeout', config.timeout)
    }
    return config;
  },
  function (error) {
    const { config } = error;
    if(config !== undefined && (config.waitOn != false && !config.fromPopup)) {
      waitOff('contentInner-' + config.gridId);
    } else {
      waitOff('body');
    }
